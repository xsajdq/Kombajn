...
export function BillingPage(): TemplateResult {
        ...
        return html`
            <div class="border-2 p-4 rounded-lg text-center flex flex-col ${isCurrent ? 'border-primary ring-2 ring-primary' : 'border-border-color'}">
                ...
                <ul class="space-y-2 my-4 text-left text-sm flex-grow">
                    ${features.map(f => html`<li class="flex items-center gap-2"><span class="material-icons-sharp text-success text-base">check_circle</span> ${f}</li>`)}
                </ul>
                <button class="w-full mt-4 px-3 py-2 text-sm font-medium rounded-md ${isCurrent ? 'bg-content border border-border-color cursor-default' : 'bg-primary text-white hover:bg-primary-hover'}" data-plan-id="${planId}" ?disabled=${isCurrent}>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                     ...
                     <div class="bg-content p-5 rounded-lg shadow-sm">
                        <table class="w-full text-sm">
                            ...
                            <tbody class="divide-y divide-border-color">
                                ${(activeWorkspace.planHistory || []).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()).map(change => html`
                                    <tr>
             ...
             <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
                    ${(Object.keys(PLANS) as PlanId[]).map(renderPlanCard)}
                </div>
            </div>
        </div>
    `;
