...
export function AIAssistantPage(): TemplateResult {
    ...
    let resultsContent: TemplateResult;
    if (loading) {
        resultsContent = html`
        ...
        resultsContent = html`<div class="p-4 text-center text-danger bg-danger/10 rounded-md">${error}</div>`;
        ...
        if (suggestedTasks.length === 0) {
             ...
             resultsContent = html`
                ...
                <div class="divide-y divide-border-color">
                    ${suggestedTasks.map((task, index) => html`
            ...
            <div class="bg-content p-6 rounded-lg shadow-sm border border-border-color">
                <form id="ai-task-generator-form" class="space-y-4">
                    ...
                    <div>
                        ...
                        <select id="ai-project-select" class="w-full bg-background border border-border-color rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-primary focus:border-primary outline-none transition" ?disabled=${workspaceProjects.length === 0}>
                             ${workspaceProjects.length > 0 ?
                                workspaceProjects.map(p => html`<option value="${p.id}">${p.name}</option>`) :
                                html`<option>${t('ai_assistant.project_select_empty')}</option>`
                             }
                        </select>
                    </div>
                    <button type="submit" class="w-full sm:w-auto px-4 py-2 text-sm font-medium flex items-center justify-center gap-2 rounded-md bg-primary text-white hover:bg-primary-hover disabled:bg-primary/50 disabled:cursor-not-allowed" ?disabled=${workspaceProjects.length === 0 || loading}>
