...
import { AddCommentToTimeLogModal } from './AddCommentToTimeLogModal.ts';
import { TaskDetailModal } from './TaskDetailModal.ts';
...
interface UserSelectOptions extends ControlOptions {
    users: User[];
    unassignedText?: string;
}

function renderUserSelect(opts: UserSelectOptions): string {
    const selectedUser = opts.value ? opts.users.find(u => u.id === opts.value) : null;
    const unassignedText = opts.unassignedText || t('tasks.unassigned');
    return `
        <div class="${opts.containerClassName || formGroupClasses}">
            <label class="${labelClasses}">${opts.label}</label>
            <div class="relative custom-select-container" id="${opts.id}-container" data-field-key="${opts.id}">
                <input type="hidden" id="${opts.id}" name="${opts.id}" value="${opts.value || ''}">
                <button type="button" class="form-control text-left flex justify-between items-center custom-select-toggle">
                    <span class="custom-select-display flex items-center gap-2">
                        ${selectedUser
                            ? `<div class="avatar-small">${getUserInitials(selectedUser)}</div><span>${selectedUser.name || selectedUser.initials}</span>`
                            : `<span>${unassignedText}</span>`
                        }
                    </span>
                ...
                <div class="custom-select-dropdown hidden">
                    <div class="custom-select-list">
                        <div class="custom-select-option ${!opts.value ? 'bg-primary/10' : ''}" data-value="">
                            <span>${unassignedText}</span>
                        </div>
                        ${opts.users.map(u => `
                            <div class="custom-select-option ${opts.value === u.id ? 'bg-primary/10' : ''}" data-value="${u.id}">
...
function renderMultiUserSelect(opts: UserSelectOptions & { selectedUserIds: string[] }): string {
    const selectedUsers = opts.selectedUserIds.map(id => opts.users.find(u => u.id === id)).filter(Boolean);
    return `
        <div class="${opts.containerClassName || formGroupClasses}">
            <label class="${labelClasses}">${opts.label}</label>
            <div id="${opts.id}" class="multiselect-container" data-type="assignee">
                <div class="multiselect-display">
                    ${selectedUsers.length > 0
                        ...
                        `).join('')
                        : `<span class="subtle-text">${opts.unassignedText || t('modals.unassigned')}</span>`
                    }
                </div>
                <div class="multiselect-dropdown hidden">
                    ...
                    ${opts.users.map(user => `
                        <label class="multiselect-list-item">
                            <input type="checkbox" name="${opts.id}" value="${user!.id}" ${opts.selectedUserIds.includes(user.id) ? 'checked' : ''}>
                            <div class="avatar">${getUserInitials(user)}</div>
                            <span>${user!.name || user!.email}</span>
                        </label>
                    `).join('')}
...
export function Modal() {
    ...
    if (state.ui.modal.type === 'addExpense') {
        ...
        body = `
            <form id="addExpenseForm" class="space-y-4">
                ${renderTextInput({ id: 'expenseDescription', label: t('modals.expense_description'), required: true })}
                <div class="${modalFormGridClasses}">
                    ${renderTextInput({ id: 'expenseAmount', label: t('modals.expense_amount'), type: 'number', required: true, min: 0, step: 0.01 })}
                    ${renderTextInput({ id: 'expenseDate', label: t('modals.expense_date'), type: 'date', required: true, value: new Date().toISOString().slice(0, 10) })}
                </div>
                <div class="${modalFormGridClasses}">
                    ${renderSelect({
                        id: 'expenseProject', label: t('budget.modal_expense_project'),
                        options: [{ value: '', text: '--' }, ...workspaceProjects.map(p => ({ value: p.id, text: p.name }))]
                    })}
                    ${renderTextInput({ id: 'expenseCategory', label: t('budget.modal_expense_category'), placeholder: 'e.g., Software, Travel' })}
                </div>
                <div class="flex items-center gap-2">
                    <input type="checkbox" id="expenseIsBillable" class="h-4 w-4 rounded text-primary focus:ring-primary">
                    <label for="expenseIsBillable" class="text-sm font-medium">This is a billable expense (will be added to client invoice)</label>
                </div>
            </form>
        `;
    }
    ...
    if (state.ui.modal.type === 'setBudgets') {
        ...
        const period = modalData.period || new Date().toISOString().slice(0, 10);
        const budgetsForPeriod = state.budgets.filter(b => b.period === period);
        body = `
            <form id="setBudgetsForm" class="space-y-4" data-period="${period}">
                <div class="grid grid-cols-[1fr,auto] gap-2 font-semibold text-sm">
                ...
                <div id="budgets-container" class="space-y-2">
                    ${budgetsForPeriod.map(budget => `
                        <div class="grid grid-cols-[1fr,auto] gap-2 items-center" data-budget-id="${budget.id}">
                            <input type="text" class="${formControlClasses}" name="budget_category_${budget.id}" value="${budget.category}" readonly>
                            <input type="number" class="${formControlClasses} w-32 text-right" name="budget_amount_${budget.id}" value="${budget.amount}" min="0" step="0.01">
                        </div>
                    `).join('')}
                </div>
                <button type="button" id="add-budget-category-btn" class="btn btn-secondary btn-sm"><span class="material-icons-sharp text-base">add</span> ${t('budget.modal_add_category')}</button>
            </form>
        `;
    }
