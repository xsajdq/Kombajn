            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    ...
                    <div class="relative custom-select-container" data-filter-key="assigneeId" data-current-value="${filters.assigneeId}">
                        ...
                        <button type="button" class="form-control text-left flex justify-between items-center custom-select-toggle">
                            <span class="custom-select-display flex items-center gap-2">
                                ${selectedAssignee
                                    ? html`<div class="avatar-small">${getUserInitials(selectedAssignee)}</div><span>${selectedAssignee.name || selectedAssignee.initials}</span>`
                                    : html`<span>${t('tasks.all_assignees')}</span>`
                                }
                            </span>
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select id="task-filter-priority" class="form-control" data-filter-key="priority">
                        ...
                        <option value="low" ?selected=${filters.priority === 'low'}>${t('tasks.priority_low')}</option>
                        <option value="medium" ?selected=${filters.priority === 'medium'}>${t('tasks.priority_medium')}</option>
                        <option value="high" ?selected=${filters.priority === 'high'}>${t('tasks.priority_high')}</option>
                    </select>
                </div>
                <div class="flex flex-col gap-1.5">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <select id="task-filter-date" class="form-control" data-filter-key="dateRange">
                        ${dateRanges.map(dr => html`<option value="${dr}" ?selected=${filters.dateRange === dr}>${t(`tasks.date_${dr}`)}</option>`)}
                    </select>
                </div>
            ...
            <div class="pt-4 border-t border-border-color flex flex-wrap justify-between items-center gap-4">
                <div class="flex items-center gap-2">
                    <label class="flex items-center gap-2 text-sm text-text-subtle cursor-pointer">
                        <input type="checkbox" id="task-filter-archived" class="h-4 w-4 rounded text-primary focus:ring-primary" ?checked=${filters.isArchived} data-filter-key="isArchived">
